...
/*  Structure of the operation supercube for 2 cubes */

int custom_supercube_2 ( 
    ...
    
{
	...
	int i;
   	...
   	const char allowed_characters[] = "01";
		...
		supercubeResult = ( char * ) malloc ( longestString + 1); 

		//Check for errors in malloc.

		if (supercubeResult == NULL)
			{
				printf ("Failed To Allocate Memory!");
				return TCL_ERROR;
			}
		...
		//supercube operation is similar to bitwise OR operation. The following loop produces the result.
		...
		for ( i = 0; i < longestString + 1; i++ )
		{
		  ...
		  if ( ( *(cube1 + i) == '1' ) || ( *(cube2 + i) == '1' ) )
		  {
			...
			strcpy(supercubeResult + i , "1"); 

		  }
		  else if (( *(cube1 + i) == '0' ) && ( *(cube2 + i) == '0' ) )
		  {
		  	
		  	strcpy(supercubeResult + i, "0"); 

		  }
		} 

		//Set result of supercube as Tcl Result, in order to appear in the cell.

		Tcl_SetResult (interp, supercubeResult, TCL_VOLATILE);

		//Free unneeded allocated space.

		free(cube1);
		free(cube2);
	 	free(supercubeResult);
